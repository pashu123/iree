...
struct Convert5DUnPackto4DUnPackPattern
  ...
  LogicalResult matchAndRewrite(tensor::UnPackOp unpackOp,
    ...
    int64_t destPos = 0;
    llvm::SmallDenseSet<int64_t> s;
    s.insert(unpackOp.getInnerDimsPos().begin(),
             unpackOp.getInnerDimsPos().end());
    SmallVector<int64_t> outerDimsPermOrSeq =
        llvm::to_vector(llvm::seq<int64_t>(0, unpackOp.getDestRank()));
    if (!unpackOp.getOuterDimsPerm().empty())
      applyPermutationToVector(outerDimsPermOrSeq, unpackOp.getOuterDimsPerm());

    for (auto [idx, val] : llvm::enumerate(outerDimsPermOrSeq)) {
      if (s.contains(val))
        continue;
      srcPos = idx;
      destPos = val;
